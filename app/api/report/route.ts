...
async function notifyOperator(report: {
  ...
  try {
    await enqueueOutbox(prisma, {
      type: "REPORT_EMAIL",
      payload: {
        // The notification goes to the one published mailbox, so the address a
        // report is mailed to cannot differ from the one the contact page and
        // `/faq` tell reporters to write to.
        to: process.env.REPORT_NOTIFY_EMAIL ?? SUPPORT_EMAIL,
        id: report.id,
