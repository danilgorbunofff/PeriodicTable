...
const sign = (
  ...
  const key = Buffer.from(
    (opts.secret ?? SECRET).replace(/^whsec_/, ""),
    "base64",
  );
  const mac = createHmac("sha256", key)
    .update(`${id}.${stamp}.${body}`, "utf8")
    .digest("base64");
  return new Headers({
    "svix-id": id,
...
const fresh = (body: string) =>
  sign({ body, stamp: Math.floor(Date.now() / 1000) });
/** Audit rows name the address inside a longer sentence, so cleanup matches on
 *  containment rather than equality. */
const AUDIT_WHERE = {
  OR: ADDRS.map((address) => ({ detail: { contains: address } })),
};
const clean = async () => {
...
describe("the suppression matrix", () => {
  it("list mail stops for every reason; money and operator mail only for undeliverable ones", () => {
    for (const reason of [
      ...
      expect(refuses(reason, "list"), `${reason} must stop list mail`).toBe(
        true,
      );
    }
    ...
    for (const reason of ["unsubscribe", "complaint", "manual"] as const) {
      expect(
        refuses(reason, "account"),
        `${reason} must not stop a reversal notice`,
      ).toBe(false);
      expect(
        refuses(reason, "internal"),
        `${reason} must not stop operator mail`,
      ).toBe(false);
    }
  ...
  it("addresses are normalized once, at the edge", () => {
    expect(normalizeRecipient("  Someone@Example.COM ")).toBe(
      "someone@example.com",
    );
...
describe("provider events decide suppression", () => {
  it("only evidence that the address is unusable, or that the person objected", () => {
    expect(resendEventEffect("email.bounced", "Permanent")).toMatchObject({
      suppress: "bounce",
    });
    ...
    expect(resendEventEffect("email.something_new")).toMatchObject({
      suppress: null,
      note: "email.something_new",
    });
  });
});
...
describe("webhook signature verification (Svix scheme)", () => {
  ...
  it("refuses everything else", () => {
    expect(
      verifyResendWebhook(BODY, sign(), NOW + WEBHOOK_TOLERANCE_SECONDS + 1),
    ).toBe(false);
    ...
    expect(
      verifyResendWebhook(
        BODY,
        sign({ secret: "whsec_AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA" }),
        NOW,
      ),
    ).toBe(false);
    expect(
      verifyResendWebhook(BODY, sign({ stamp: "not-a-number" }), NOW),
    ).toBe(false);
    ...
    wrongVersion.set(
      "svix-signature",
      wrongVersion.get("svix-signature")!.replace("v1,", "v2,"),
    );
...
describe.skipIf(!hasDb)("refusals live on the address", () => {
  it("records, re-reasons, and never lets a provider signal narrow the person's own", async () => {
    ...
    expect(
      await prisma.emailAddress.count({ where: { email: LIST_ADDR } }),
    ).toBe(1);
    ...
    expect(await resolveUnsubTarget(token)).toEqual({
      kind: "address",
      email: LIST_ADDR,
    });
  ...
  it("stops list mail but still sends the money notice (§9 Q1)", async () => {
    ...
    const logged = await prisma.emailLog.findUniqueOrThrow({
      where: { id: waitlist.emailLogId! },
    });
    ...
    const refundRow = await prisma.emailLog.findUniqueOrThrow({
      where: { id: refund.emailLogId },
    });
...
describe.skipIf(!hasDb)(
  "what the provider's answer is kept for (R10-1, R10-11)",
  () => {
    ...
    it("records acceptance with the provider's message id, refusal with its words", async () => {
      // R13-8: capture what the provider is actually sent — the key on the mail
      // and the key in the register must be the same string, or a retry after a
      // lost response mails the buyer twice.
      const sent: Record<string, string>[] = [];
      vi.stubGlobal(
        "fetch",
        async (_url: string, init: { headers: Record<string, string> }) => {
          sent.push(init.headers);
          return new Response(JSON.stringify({ id: "msg_ok_1" }), {
            status: 200,
          });
        },
      );
      ...
      const refused = await sendReceiptEmail({
        ...payload,
        dedupeKey: "sup-provider-key-2",
      });
      ...
      expect(sent.map((h) => h["Idempotency-Key"])).toEqual([
        mailIdempotencyKey("sup-provider-key"),
        mailIdempotencyKey("sup-provider-key-2"),
      ]);
    });
  },
);
...
describe.skipIf(!hasDb)("the Resend webhook route (R10-7)", () => {
  ...
  it("suppresses a permanent bounce, names it, and annotates the row we sent", async () => {
    ...
    const row = await prisma.emailAddress.findUniqueOrThrow({
      where: { email: HOOK_ADDR },
    });
    ...
    const annotated = await prisma.emailLog.findUniqueOrThrow({
      where: { id: sent.id },
    });
  ...
  it("answers 200 for a transient bounce and for an event it does not know, without suppressing", async () => {
    ...
    expect(
      await (await post(transient, fresh(transient))).json(),
    ).toMatchObject({ acted: false });
  ...
  it("rejects a body that is not JSON, and one that names no event", async () => {
    ...
    expect(
      await (await post(noRecipient, fresh(noRecipient))).json(),
    ).toMatchObject({
      ok: true,
      acted: false,
    });
