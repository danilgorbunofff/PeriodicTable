...
> **Amended 2026-09-16 (PR `26` audit):** the path has since grown two server gates this walk predates: `checkMoneyPath()` (`lib/moneyPath.ts:33`, called at `app/api/checkout/route.ts:185`) answers **503** when the deployment wants money but cannot service the charge (`:197` — the R14-1(b) posture), and `attestVersionRefusal()` (`lib/consent.ts:53`, called at `app/api/checkout/route.ts:223`) answers **409 `RULES_UPDATED`** (`:228`) when the buyer consented to a superseded rule version. Neither changes what the walk above records; both sit between intake and the provider call.
...
- 2026-09-16 (working tree, PR `26` final-verification audit) — line-refresh amendment in §4: the two server gates the walk predates — `checkMoneyPath()` answering 503 and `attestVersionRefusal()` answering 409 `RULES_UPDATED` — noted with today's citations (`lib/moneyPath.ts:33`, `lib/consent.ts:53`, `route.ts:185/:197/:223/:228`).
