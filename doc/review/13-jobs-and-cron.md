...
**Scope correction to the plan.** §2 of `00-REVIEW-PLAN.md` opens this doc with "`vercel.json` schedules exactly two jobs … while the codebase also exposes `/api/jobs/reconcile` and `/api/jobs/config`". `vercel.json` does schedule exactly two (`vercel.json:3-5`), but it is **not the only scheduler**: `.github/workflows/outbox-tick.yml:10` schedules `*/10 * * * *` and calls **all four** job routes, including `reconcile` and `config` (`:44,50,56,78-98`). `gh workflow list` shows both workflows `active`. The plan's framing is therefore incomplete in the same way its "dead Turnstile entries" phrase is (`14` §5 corrects that one) — the doc answers the questions the plan asks, from the full inventory rather than the two-cron subset.

**Paths inspected.** `vercel.json:1-7`; `.github/workflows/outbox-tick.yml` (whole file, 98 lines); `.github/workflows/ci.yml:60-95`; `app/api/jobs/outbox/route.ts` (41 lines); `app/api/jobs/screenshot/route.ts` (71 lines); `app/api/jobs/reconcile/route.ts` (101 lines); `app/api/jobs/config/route.ts` (40 lines); `app/api/admin/outbox/retry/route.ts` (31 lines); `lib/jobs.ts` (41 lines); `lib/outbox.ts` (221 lines); `lib/email.ts:1-100`; `lib/env.ts:1-170`; `lib/screenshots.ts:55-92`; `lib/settle.ts:70-80,275-290`; `app/api/report/route.ts:70-79`; `app/api/waitlist/route.ts:60-68`; `lib/stripe.ts:118-126`; `README.md:145-160`; `HANDOFF.md:125-140,800-830`; `doc/PROD-READINESS-CHECKLIST.md` §4b, §7.
...
| `GET`/`POST /api/jobs/daily` | Vercel cron `30 4 * * *` (`vercel.json:5`) — the entry that used to be screenshot's (R20-7); **no** tick step calls it | Same | Preview drain within `DAILY_PREVIEW_BUDGET_MS`, then config + reconcile in-process (`route.ts:70-79`) | Drain claims like the workers; both read-only legs re-run free | `maxDuration` 30 s (`route.ts:12`), the drain budgeted inside it | Any failing leg → **500** naming the count and carrying each leg's report (`route.ts:81-99`) | Vercel dashboard only — no step watches it; each leg still stamps its own heartbeat (`route.ts:70-82`)

**One row above is a correction, and it is the inventory's own failure mode.** The sweep has no counterpart in the version of this doc that was authored: `/api/jobs/abandoned-checkouts` arrived in `08`'s fix pack (`6e4eca7`), after this doc was written against `9681bdc`, and nothing in the table's own rules would have caught its absence — which is what §8's last acceptance box is about. Its cells are the post-fix reading (§5.15); the other five rows are as measured on 2026-09-15, and §5.15 records where the fix pass moved their line numbers and bodies. The `daily` row is the same failure mode caught later still: R20-7 (`20` §5.6) replaced screenshot's `30 4` cron entry with the composite, and its cells are the current reading, not a 2026-09-15 measurement.
...
All four job routes are indistinguishable to an anonymous caller, which is the property `11` §6 asserts. (The `?secret=wrong` line was measured before `14` R14-8 deleted the query form on all job routes; the answer is the same `401` either way.) The same matrix on the **dev** server `:3215` answers `200` for `outbox`, `screenshot` and `reconcile`, and `503` for `config` (its findings, not its auth) — because `jobAuth` returns `null` rather than a 401 when the environment is not production (`lib/jobs.ts:25-28`). That is R13-2.
...
| What is the operator cadence? | **`*/10` nominal, ~3 h in practice.** `.github/workflows/outbox-tick.yml:10` is the only cadence in the repository; the app-side paths are the daily 04:00/04:30 backstop. `lib/env.ts:137` names "every 10-minute tick" as the intended alert cadence, and `HANDOFF.md:807-821` records the 2026-09-11 decision that the external pinger is optional and the daily Vercel pair is the floor. The measured cadence is in §5.6. |
...
| --- | --- | --- |
| `vercel.json:3-6` | 2 crons: `/api/jobs/outbox` `0 4 * * *`, `/api/jobs/screenshot` `30 4 * * *` — **superseded by R20-7**: the second entry is now `/api/jobs/daily` (`vercel.json:4-5` today) | those 2 paths, the composite's legs in-process |
| `.github/workflows/outbox-tick.yml:10` | `*/10 * * * *` + `workflow_dispatch` | `screenshot` (backfill, manual only), `outbox`, `screenshot`, `config`, `reconcile` |
| `app/api/jobs/*` | 4 routes — **6 today**: `abandoned-checkouts` (`08`'s pack) and `daily` (R20-7) landed after this row was written | — |
| `/api/admin/outbox/retry` | 1 route, no scheduler | operator only |

So four routes are reachable by two schedulers and one human; `reconcile` and `config` are reached by exactly one of the three, and that one is the scheduler whose cadence is 5.1 % of nominal. Six routes today — `daily` is reachable by Vercel only, `abandoned-checkouts` by the tick only, and neither is watched by the other scheduler.

### 5.14 Vercel cron behaviour, as declared

`vercel.json:1-7` sets no `crons` options beyond the two entries (no `path` variants, no per-cron timezone). Whatever Vercel does with a failing cron invocation is platform behaviour this review cannot observe from the checkout — **U13-2** and **U13-5** carry the residue, and `15` §12 records the cost consequence of a run that is killed rather than completed.
...
**Every line number and body in §5.1–§5.14 is the at-authoring one.** `outbox` and `screenshot` gained `maxDuration` and moved their constants into `lib/jobBudget.ts`, so the `route.ts:6,25-26` citations are historical; their bodies gained `errors / skipped / deferred / batches / remaining` and `config`'s gained `heartbeats`, so the sampled bodies in §5.8 and §5.13 are the shape *before* the pass; `lib/jobs.ts:19-31` is now `:40-57`; and the inventory listed five rows for a tree that has six — the sixth was added by this pass. A seventh arrived after it: R20-7 (`20` §5.6) replaced screenshot's `30 4` cron entry with the `/api/jobs/daily` composite (`vercel.json:5`), which drains previews inside `DAILY_PREVIEW_BUDGET_MS`, then runs config and reconcile in-process and answers 500 with each leg's report (`route.ts:81-99`) — §5.1's `daily` row is the current reading. The cadence measurement itself — §5.6's 34 runs, 5.1 %, median 3 h 12 m, worst 6 h 40 m, 26 consecutive green since 2026-09-11T13:48:12Z — is unchanged, and is now the *input* to the stale bounds rather than only a finding.
...
- `14` owns exploitability: R13-2's fail-open auth is the queue-draining path to the free stake chain `14` R14-1 describes; the query-string `?secret=` form (`reconcile/route.ts:47`, `config/route.ts:35`) is a token-in-URL habit that reaches access logs, registered there — **`14` R14-8 deleted both reads** (bearer header or JSON body `secret` only), so this doc's §5.3 transcript is the record of the pre-fix shape.
...
- 2026-09-16 (working tree) — re-verification fix, five citations that named lines their files did not have. §1's "Paths inspected" list: `lib/screenshots.ts:55-100` → `:55-92`, `app/api/report/route.ts:70-80` → `:70-79`, `app/api/waitlist/route.ts:60-70` → `:60-68`. §1 and §5.14 both cite `vercel.json:1-12` → `vercel.json:1-7`: the file is seven lines at every commit in this history, and `:3-6` is the `crons` array the sentence is about, so the correction is the whole file rather than a range inside it. No claim changed.
...
- 2026-09-16 (working tree, PR `26` final-verification audit) — F-009: §7 R13-6's screenshot citation `:99-115` → `:88-98`; the 107-line file never had those lines — the 500-with-counts block is `:88-98` — and FINDINGS.md's R13-6 row carried the same slip. §5.1 gains the `/api/jobs/daily` row and its screenshot cell names the R20-7 composite; §5.13, §9 Q3 and U13-2 note the supersession; the `*/10` citation `outbox-tick.yml:9` → `:10`; and the three `******` redaction artifacts in §5.1's auth cells (and the actor row's) are restored to their plain wording — `Bearer CRON_SECRET`, `Authorization: Bearer (ADMIN_TOKEN)` — the §5.2 transcript's `Authorization: ******` is left as the record it is.
- 2026-09-16 (working tree, PR `26` final-verification audit) — line refresh: both R13-2 `lib/env.ts:60-73` citations annotated with the predicate's today-line (`:71`); the def moved as `lib/env.ts` grew.
...
| U13-2 | Whether the two `vercel.json` crons actually fire on the current deployment, and what Vercel does with a failing invocation (retry, or nothing). Read-only, needs production access this checkout does not have. | Vercel dashboard `Project → Settings → Cron Jobs` history for `periodictable.lol`, or `vercel crons ls` with a working token. *Unchanged by the fix pass, and now self-answering if it is:* the entries are identifiable in the dashboard by their query strings (`?limit=25`; the `?limit=10` entry became `/api/jobs/daily` — R20-7), and a firing entry stamps the same heartbeat the GitHub tick does — so if they fire, `outbox` and `screenshot` will show stamps far newer than 24 h apart |
