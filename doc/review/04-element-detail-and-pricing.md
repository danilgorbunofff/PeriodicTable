...
| 5.7 | Repo-wide grep `validateTake` | only `lib/pricing.test.ts:11,93-95` reference it (`lib/pricing.ts:53-58` at authoring; `validateTake` is `:77-83` today) |
...
- Evidence: `validateTake` (`lib/pricing.ts:53-58` at authoring; `:77-83` today) is referenced only by its own test (§5.7); the floor lives in `reservedTotal` and `lib/settle.ts:161` (the `take-below-reserve` check is `:245-247` today).
...
- 2026-09-16 (working tree, PR `26` final-verification audit) — line refresh: `validateTake` `:53-58` → `:77-83`, the settle floor `:161` → `:245-247`, the checkout call `:309-312` → `:446-448`, `lib/api.ts` `boardComplete` `:126` → `:140`.
