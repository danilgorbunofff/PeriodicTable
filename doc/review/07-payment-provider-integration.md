...
7. **Simulator.** In dev mode the buyer instead lands on `/pay/<id>`, a page whose only control is the simulator (`PaySimulator.tsx:9-108`): *Pay now* → `POST /api/dev/pay` → on `status === "paid"` it routes to `/?paid=<SYM>`, otherwise it prints `json.error ?? json.status` (`:21-34`).
...
- 2026-09-16 (working tree, PR `26` final-verification audit) — R07-1/R07-3 amendments: the simulator route's outer production 404 and 30/hour budget (`app/api/dev/pay/route.ts:45-59`), the startup report's emit line today (`lib/prisma.ts:13`), and §3's env citations refreshed (`lib/env.ts:34`, `:48`, `:363`).
- 2026-09-16 (working tree) — re-verification fix. §4's simulator citation, `PaySimulator.tsx:77-113`, named a range the file did not have: the component is 108 lines, so `:113` did not exist when the sentence was written. Re-anchored to `PaySimulator.tsx:9-108` — the component's own span, which carries every clause of the sentence: the `POST /api/dev/pay` at `:17`, the `status === "paid"` route to `/?paid=<SYM>` at `:25-27`, and the two-button control row at `:67-98`. No claim changed.
