...
| 3 | A **report** is filed (`app/api/report/route.ts:60-79`) | `report` — `emails/report.tsx` | the moderation inbox from env | `report-mail:${report.id}` (`app/api/report/route.ts:72`) | Not applicable — one-off operational copy to the operator, no `List-Unsubscribe` by design (`lib/email.ts:140-141`) |
...
- **R10-6 — the waitlist confirmation carries an opt-out and can be suppressed.** The message now
  ...
  (`lib/email.ts:650-664`, footer link at `emails/waitlist.tsx:23-25`), and the join is recorded but
...
- 2026-09-16 (working tree) — fix pass for R10-1…R10-9 and R10-11, each cited in §7 with its verification
  ...
  **U10-1…U10-6 stand unchanged** — the pass added no DNS record, sent no mail to a real inbox, read no
  ...
  nothing.
- 2026-09-16 (working tree) — re-verification fix, one class in six citations: each named a line its file did not have. §1's "Paths inspected" list: `app/api/report/route.ts:40-80` → `:40-79`. §3.1's inventory table: `app/api/report/route.ts:60-80` → `:60-79` (`notifyOperator`, the `enqueueOutbox`/`drainDueWithin` pair the row describes). §5.5 and R10-8's evidence: `app/api/emails/preview/route.ts:44` → `:36` — the file was 38 lines, and `:36` is the source line that sets `X-Subject-Preview`, while 44 is the transpiled frame `11` §5.8 quotes as a frame. §5.7: `lib/email.ts:650-668` → `:650-664` (`sendWaitlistEmail` runs to the file's last line). R10-5: `app/api/unsubscribe/route.ts:53-70` → `:53-68`. No claim, count or verdict changed; §5.7's fix-verification figures are untouched.
