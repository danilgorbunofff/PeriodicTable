...
- [ ] **Prove the automatic tick actually fires** — *a manual dispatch only proves auth, never the timer.* `gh run list --workflow=outbox-tick.yml --limit 20 --json event,createdAt` must show a run with `event: "schedule"`. **Status 2026-09-10 12:10Z: still never fired.** Everything on GitHub's side checks out — workflow registered `state:active` at 10:45:16Z, file on the default branch `main`, `fork:false`, `disabled:false`, cron `*/10 * * * *` valid — yet the repo-wide run API (`actions/runs?per_page=100`) returns **33 runs: 17 push + 10 pull_request + 6 workflow_dispatch, and 0 `schedule`**. That is **1 h 25 m and 9 consecutive missed boundaries** (10:50 → 12:10Z). GitHub crons are best-effort by design, but this is no longer "a few minutes late" — **treat the GitHub timer as unproven and do not rely on it.** Fallback: add a second, independent 10-min scheduler (cron-job.org can send the `Authorization: Bearer <CRON_SECRET>` header to `/api/jobs/outbox` and `/api/jobs/screenshot`), or accept the daily `vercel.json` backstop as the only automatic path. Auth is verified ready for an external pinger: both paths accept a plain **GET** with `Authorization: Bearer …` (`jobAuth` strips the `Bearer ` prefix, no body needed), and a deliberately wrong secret returns **401** on both — so a 200 from cron-job.org is real proof, not a false positive. **Blast radius if the tick is late (why this is not launch-blocking):** receipts/outbid mail is sent **inline** at webhook time, so the outbox is the *retry* path only, and previews render nowhere in v1 — a missed tick delays a retry, it never silences first-time mail.
