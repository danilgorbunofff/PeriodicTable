...
- [ ] **Prove the automatic tick actually fires** — *a manual dispatch only proves auth, never the timer.* `gh run list --workflow=outbox-tick.yml --limit 20 --json event,createdAt` must show a run with `event: "schedule"`. (2026-09-10 11:29Z: everything on GitHub's side checks out — workflow registered `state:active` at 10:45:16Z, file is on the default branch `main`, `fork:false`, `disabled:false`, cron `*/10 * * * *` valid — yet across all **29** runs there were **0** `schedule` events, 44 min and ~4 expected ticks later. GitHub crons are explicitly best-effort: a newly registered schedule can take up to ~an hour to start, and queued ticks get dropped under load. ⏳ re-check pending.) **If it never fires, do not call the timer a safety net:** either add a second, independent 10-min scheduler (cron-job.org can send the `Authorization: Bearer` header) or accept the daily `vercel.json` backstop as the only automatic path. Note the blast radius if the tick is late: receipts/outbid mail are sent **inline** at webhook time, so the outbox is the *retry* path, and previews render nowhere in v1 — a missed tick delays retries, it does not silence first-time mail.
